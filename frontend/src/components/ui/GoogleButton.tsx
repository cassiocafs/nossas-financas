...
export function GoogleButton({ label, className = "", type = "button", ...props }: GoogleButtonProps) {
  return (
    <button
      type={type}
      className={`inline-flex h-12 w-full items-center justify-center gap-2 rounded-md border border-border bg-background text-[14px] font-semibold text-foreground transition-colors hover:bg-accent disabled:cursor-not-allowed disabled:opacity-50 ${className}`}
      {...props}
    >
